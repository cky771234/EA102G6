...
public class ANNImg extends HttpServlet{
...
private static final String GETONEPIC ="SELECT ANN_IMG FROM ANNOUNCEMENT WHERE ANN_NO=?";
 Connection con;
 public void doGet(HttpServletRequest req, HttpServletResponse res)
  ...
  try {
   ...
   String annno = req.getParameter("annno").trim();
   pstmt.setString(1, annno);
   ResultSet rs = pstmt.executeQuery();
   
   if (rs.next()) {
    BufferedInputStream in = new BufferedInputStream(rs.getBinaryStream("ANN_IMG"));
